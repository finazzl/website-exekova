...
const run: SignInRunData = {
  ...
  sources: [
    ...
    { name: 'Slack', logo: logo('slack.png'), note: 'Requests' },
  ...
  steps: [
    ...
    { short: 'Execute', status: 'Executing', tone: 'run', tools: [{ name: 'Claude Code', logo: logo('claude-code.png'), note: 'Selected for implementation' }] },
    { short: 'Verify', status: 'Gate held', tone: 'held', tools: [{ name: 'Codex', logo: logo('codex.png'), note: 'Independent review' }, { name: 'Test runner', icon: 'flask', note: 'Required CI evidence' }] },
    { short: 'Recover', status: 'Recovering', tone: 'run', tools: [{ name: 'Claude Code', logo: logo('claude-code.png'), note: 'Repair on the new revision' }, { name: 'Test runner', icon: 'flask', note: 'Retest within the ceiling' }] },
...
export default function SignInPage() {
  const site = getSite();
  return <div className="beta-page signin-main"><SignIn wordmark={site.brand.wordmark as string} mark="/brand/exekova-mark.png" run={run}/></div>;
}
