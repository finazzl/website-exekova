...
const backdrop = 'workspace-flow' as const;
...
export function WorkBackdrop({showcase=false}:{showcase?:boolean}) {
  return <><ResponsivePhoto className="work-backdrop" name={backdrop} alt="" sizes={showcase ? '(max-width: 700px) 100vw, 1160px' : '(max-width: 700px) 100vw, 400px'}/><div className="work-shade"/></>;
}
  ...
  return <div className="work-transformation" data-step={step} data-complete={complete} data-showcase={showcase}>
    ...
    <div className="work-scene-content">
      ...
      <div className="work-document" key={step}>
        <div className="work-document-bar"><Image src="/brand/exekova-mark.webp" width={24} height={24} alt=""/><span>{step===0 ? 'Task received' : step===1 ? 'Repository selected' : complete ? 'Verified outcome' : 'Checking the outcome'}</span>{step===2 && complete && <Icon name="check" size={16}/>}</div>
    ...
    <div className="work-brand-pill"><Image src="/brand/exekova-mark.webp" width={28} height={28} alt=""/><span>exekova</span><i/><i/><i/></div>
