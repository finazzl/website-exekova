...
function FeatureVisual({ index }: { index: number }) {
  return <div className="feature-visual" data-feature={index}>
    ...
    index === 2 ? <ReviewDemo/> :
    <div className="feature-evidence"><div><Image src="/brand/exekova-mark.webp" width={32} height={32} alt=""/><span>exekova</span><span className="feature-verified"><Icon name="check" size={14}/>Verified</span></div><h3>Checkout validation fixed.</h3><ul>{['Independent review passed', 'Required checks passed', 'Acceptance criteria met'].map(item => <li key={item}><Icon name="check" size={16}/>{item}</li>)}</ul><p><Icon name="branch" size={16}/>Pull request ready for your team</p></div>}
