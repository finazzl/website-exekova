...
export default function SiteFooter({ site }: { site: any }) {
  ...
  return <footer className="site-footer"><div className="shell">
    <div className="footer-intro">
      <div className="footer-brand-block"><Link href="/" aria-label="Exekova home"><Image src={site.brand.logoSmall} alt="Exekova" width={148} height={74}/></Link><p>{site.footer.statement}</p><span className="footer-category">{site.brand.category}</span><div style={{ marginTop: 14 }}><ComplianceLine/></div></div>
    ...
    <nav className="footer-links" aria-label="Footer navigation">{columns.map(column => <div className="footer-column" key={column.title}><h2>{column.title}</h2><ul className="footer-row-links"><li><Link className="footer-overview" href={column.href}>{column.overview}</Link></li>{column.links.map(link => <li key={link.href}>{link.href.startsWith('/#') || link.href.startsWith('http') ? <a href={link.href}>{link.label}</a> : <Link href={link.href}>{link.label}</Link>}</li>)}</ul></div>)}</nav>
    <nav className="footer-legal" aria-label="Legal and privacy">{(site.footer.legalLinks as NavLink[]).map(link => <Link key={link.href} href={link.href}>{link.label}</Link>)}<Link href="/cookie-settings">Cookie settings</Link></nav>
